...
export function sample2() {
    class background extends instance {
        ...
        draw() {
            ...
            for (let i = 0; i < 7; i++) {
                for (let j = 0; j < 14; j++) {
                    draw_text_transformed(j * 70 + this.x - 140, this.y + i * 70 - 40, 'moved', 'center', 45);
                }
            }

            draw_set_alpha(0.9);
            draw_set_color(color.white);
            draw_rectangle(window.variables.width / 2 - 185, window.variables.height / 2 - 65, window.variables.width / 2 + 185, window.variables.height / 2 + 52, true);
            draw_set_alpha(1);
            draw_set_color(color.black);
            draw_rectangle(window.variables.width / 2 - 186, window.variables.height / 2 - 66, window.variables.width / 2 + 186, window.variables.height / 2 + 53, false);
        }
    }
    ...
    class sub_text extends instance {
        ...
        create() {
            this.text = '화면을 클릭해서 sample.js로 돌아가보세요';
            this.time = 1;
        }
